...
func TestValidationFunctions(t *testing.T) {
	...
	config := validation.ProviderConfig{
		...
		StableIDs:  []cloud.InstanceTypeID{"datacrunch_B200_helsinki-finland-5", "massedcompute_L40_desmoines-usa-1"},
	}
...
func TestInstanceTypeFilter(t *testing.T) {
	...
	instance, err := client.CreateInstance(ctx, cloud.CreateInstanceAttrs{
		...
		FirewallRules: cloud.FirewallRules{
			EgressRules: []cloud.FirewallRule{
				{
					ID:       "test-rule1",
					FromPort: 80,
					ToPort:   8080,
			...
			IngressRules: []cloud.FirewallRule{
				{
					ID:       "test-rule3",
					FromPort: 80,
					ToPort:   8080,
