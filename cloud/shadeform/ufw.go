...
func (c *ShadeformClient) GenerateFirewallScript(firewallRules cloud.FirewallRules) (string, error) {
...
func (c *ShadeformClient) convertIngressFirewallRuleToUfwCommand(firewallRule cloud.FirewallRule) []string {
	cmds := []string{}
	portSpecs := []string{}
...
func (c *ShadeformClient) convertEgressFirewallRuleToUfwCommand(firewallRule cloud.FirewallRule) []string {
	cmds := []string{}
	portSpecs := []string{}
	if firewallRule.FromPort == firewallRule.ToPort {
		portSpecs = append(portSpecs, fmt.Sprintf("port %d", firewallRule.FromPort))
