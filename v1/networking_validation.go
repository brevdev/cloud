...
func ValidateDockerFirewallBlocksPort(ctx context.Context, client CloudInstanceReader, instance *Instance, privateKey string, port int) error {
	...
	_, _, err = sshClient.RunCommand(ctx, "docker --version")
	if err != nil {
		return fmt.Errorf("docker is not available on instance: %w", err)
	}
	...
	startDockerCmd := fmt.Sprintf(
		"docker run -d --rm --name %s -p %d:%d nginx:alpine",
		containerName, port, 80,
	)
	...
	if err := waitForDockerService(ctx, sshClient, containerName, port); err != nil {
		_, _, _ = sshClient.RunCommand(ctx, fmt.Sprintf("docker rm -f %s || true", containerName))
		return err
	}

	// Debug: show iptables rules for DOCKER-USER chain BEFORE any modification
	iptablesOut, _, _ := sshClient.RunCommand(ctx, "sudo iptables -L DOCKER-USER -n -v 2>&1 || echo 'DOCKER-USER chain not found'")
	fmt.Printf("Instance %s DOCKER-USER iptables rules BEFORE:\n%s\n", publicIP, iptablesOut)

	// TESTING ONLY: Flush DOCKER-USER rules to simulate vulnerable state (no iptables protection)
	// This reproduces the issue where Docker ports are accessible without the iptables fix
	fmt.Printf("FLUSHING DOCKER-USER rules to simulate vulnerable state...\n")
	_, _, _ = sshClient.RunCommand(ctx, "sudo iptables -F DOCKER-USER")

	// Show rules after flush
	iptablesOut, _, _ = sshClient.RunCommand(ctx, "sudo iptables -L DOCKER-USER -n -v 2>&1 || echo 'DOCKER-USER chain not found'")
	fmt.Printf("Instance %s DOCKER-USER iptables rules AFTER FLUSH:\n%s\n", publicIP, iptablesOut)

	// Now try to connect to the port from outside
	// WITHOUT iptables rules, this should SUCCEED (port accessible = vulnerability exists)
	// WITH iptables rules, this should FAIL (port blocked = secure)
	fmt.Printf("Testing external connectivity to %s:%d\n", publicIP, port)
	err = checkPortBlocked(ctx, publicIP, port)

	// Clean up: stop and remove the container
	stopDockerCmd := fmt.Sprintf("docker rm -f %s || true", containerName)
...
func waitForDockerService(ctx context.Context, sshClient *ssh.Client, containerName string, port int) error {
	for i := 0; i < 30; i++ { // Try for up to 30 seconds
		time.Sleep(1 * time.Second)

		// Check container is running
		checkContainerCmd := fmt.Sprintf("docker ps --filter name=%s --format '{{.Names}}'", containerName)
